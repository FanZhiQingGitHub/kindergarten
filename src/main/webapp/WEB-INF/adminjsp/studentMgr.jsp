		...
		<div class="studentTable">
			...
			<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<input type="text" class="layui-input" name="studentName" id="studentName" placeholder="请输入学生名称" style="margin: -20% 0 0 75%;">
					</div>
				</div>
				<div class="layui-inline">
...
<%--				</div>--%>
	...
	<table id="student" lay-filter="test" class="layui-table-cell"></table>
...
<%--			<div class="layui-inline">--%>
<%--				<input type="text" id="phone" name = "phone" placeholder="请输入学生手机号" value=""--%>
<%--				       autocomplete="off" class="layui-input" style="width: 120%">--%>
<%--			</div>--%>
<%--		</div>--%>
	</div>
	...
	<div id="type-content2" style="display: none;">
		<div class="layui-form-item">
			<label class="layui-form-label">姓名：</label>
			<div class="layui-inline">
				<input type="text" id="name2" name = "name2" placeholder="请输入学生名称" value=""
				       autocomplete="off" class="layui-input" style="width: 120%">
			</div>
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">住址：</label>
			<div class="layui-inline">
				<input type="text" id="studentAdd2" name = "studentAdd2" placeholder="请输入学生住址" value=""
				       autocomplete="off" class="layui-input" style="width: 120%">
			</div>
		</div>
<%--		<div class="layui-form-item">--%>
	...
	layui.use(['form', 'layer', 'jquery', 'layedit', 'laydate', 'element', 'tree', 'table'], function () {
		...
		$(function () {
			...
			table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'forbidden') {
					layer.confirm("确定禁用学生账号？",{icon:3,title:'温馨提示'},function (index) {
						layer.close(index);
						$.ajax({
					...
					layer.confirm("确定启用学生账号？",{icon:3,title:'温馨提示'},function (index) {
						layer.close(index);
						$.ajax({
					...
					layer.confirm("确定要删除该学生信息？",{icon:3,title:'温馨提示'},function (index) {
						...
						$.ajax({
							...
							success: function (result) {
								if(result == "success")
								{
									layer.alert("学生信息删除成功！", {icon: 6});
									tableIns.reload();
								}else{
									layer.alert("学生信息删除失败！", {icon: 2});
								}
							}
						});
					});
		...
		$("#btn-add").click(function () {
			layer.open({
				...
				btn1: function (index) {
					var name = $("#name2").val();
					var add = $("#studentAdd2").val();
					// var phone = $("#phone2").val();
					if (name.length == 0) {
						layer.alert("请输入学生名称", {icon: 2});
					}  else {
						$.ajax({
							url: path + '/admin/addStudent',
							async: true,
							type: 'post',
							data: {
								"studentname": name,
		...
		$(function () {
			...
			$("#studentAdd2").blur(function () {
				var add = $("#studentAdd2").val();
				var reg = /^[\u4e00-\u9fa5]{2,20}$/;
				if(!$('#studentAdd2').val().match(reg)||add == 0)
				{
					layer.msg("请输入至少2位中文字符", {icon: 2});
				}else{
					layer.msg("输入合法",{icon:6});
