...
<input type="hidden" id="path" value="<%=path%>">
	...
	<form class="layui-form" action="" onsubmit="return false;" >
		<div class="teacherTable">
			...
			<div class="layui-form-item" style="margin-left: 6%">
				<div class="layui-inline">
					<span class="layui-form-label" style="width: auto">教师名称：</span>
					<div class="layui-input-inline">
						<input type="text" class="layui-input" name="teacherName" id="teacherName" placeholder="请输入教师名称" >
					</div>
				</div>
				...
				<div class="layui-inline" style="margin-left: 2%">
					<span class="layui-form-label" style="width: auto;">职位：</span>
	...
	<table id="teacher" lay-filter="test" class="layui-table-cell" style="margin-top: 2%"></table>

	<div id="type-content" style="display: none;">
		<div class="layui-form-item" style="margin-top: 3%">
			<label class="layui-form-label" style="width: 24%">教师名称：</label>
			<div class="layui-inline">
		...
		<div class="layui-form-item">
			<label class="layui-form-label" style="width: 24%">职位：</label>
			<div class="layui-inline">
	...
	<script type="text/html" id="barOption">
		...
		<button type="button" class="layui-btn layui-btn-danger layui-btn-xs" lay-event="forbidden" style="height: 36px;width: 18%;"><span class="sp1">禁用</span></button>
		{{#  } }}
		{{#  if(d.teacherstatus == '禁用'){ }}
		<button type="button" class="layui-btn layui-btn-danger layui-btn-xs" lay-event="open" style="height: 36px;width: 18%;"><span class="sp1">启用</span></button>
		{{#  } }}
		<button type="button" class="layui-btn layui-btn-xs" lay-event="restPwd" style="height: 36px;width: 24%;"><span class="sp1">重置密码</span></button>
		<button type="button" class="layui-btn layui-btn-xs" lay-event="update" style="height: 36px;width: 18%;"><span class="sp1">修改</span></button>
		<button type="button" class="layui-btn layui-btn-xs" lay-event="delete" style="height: 36px;width: 18%;"><span class="sp1">删除</span></button>
	</script>
</body>
<script>
	...
	layui.use(['form', 'layer', 'jquery', 'layedit', 'laydate', 'element', 'tree', 'table'], function () {
		...
		var tableIns = table.render({
			...
			, cols: [[ //表头
				...
				, {
					field: 'teacherregtime', title: '入职时间', align: 'center', width: 230
				...
				, {field: 'teacherstatus', title: '状态', align: 'center', width: 100}
				, {fixed: 'right', title: '操作', align: 'center', width: 410, toolbar: '#barOption'}
			]]
			, id: 'teacherTable'
		...
		$(function () {
			...
			table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'forbidden') {
					layer.confirm("确定禁用教师账号？",{icon:3,title:['温馨提示','font-size:18px'],area:['350px','200px'],skin:'demo'},function (index) {
						layer.close(index);
						$.ajax({
					...
					layer.confirm("确定启用教师账号？",{icon:3,title:['温馨提示','font-size:18px'],area:['350px','200px'],skin:'demo'},function (index) {
						layer.close(index);
						$.ajax({
							...
							if (name.length == 0) {
								...
								$.ajax({
									...
									success: function (result) {
										...
										{
											layer.close(index);
											layer.alert("修改教师信息成功！", {icon: 6,title:['温馨提示','font-size:18px'],area:['350px','150px'],skin:'demo'});
											tableIns.reload();
										}else{
											layer.alert("修改教师信失败！", {icon: 2,title:['温馨提示','font-size:18px'],area:['350px','150px'],skin:'demo'});
										}
									}
								});
							}
					...
					layer.confirm("确定要删除该教师信息？",{icon:3,title:['温馨提示','font-size:18px'],area:['350px','200px'],skin:'demo'},function (index) {
						layer.close(index);
						$.ajax({
							...
							success: function (result) {
								if(result == "success")
								{
									layer.alert("教师信息删除成功！", {icon: 6,title:['温馨提示','font-size:18px'],area:['350px','150px'],skin:'demo'});
									tableIns.reload();
								}else{
									layer.alert("教师信息删除失败！", {icon: 2,title:['温馨提示','font-size:18px'],area:['350px','150px'],skin:'demo'});
								}
							}
						});
					});
