	...
	layui.use(['form', 'layer', 'jquery', 'layedit', 'laydate', 'element', 'tree', 'table'], function () {
		...
		var tableIns = table.render({
			...
			, cols: [[ //表头
				...
				, {field: 'healtheradd', title: '住址', align: 'center', width: 160}
		...
		$(function () {
			...
			table.on('tool(test)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				if (layEvent === 'forbidden') {
					...
					layer.confirm("确定要修改？",{icon:3,title:'温馨提示'},function (index) {
						...
						layer.open({
							...
							success:function(){
								...
								$("#healtherAdd").val(data.healtheradd);
								$("#phone").val(data.healtherphone);
							},
							btn1: function (index) {
								var name = $("#name").val();
								...
								if (name.length == 0) {
									...
									$.ajax({
										...
										type: "post",
										data: {"healtherid": data.healtherid,"healthername":name,"healtheradd":add,"healtherphone":phone},
										dataType: "text",
										success: function (result) {
											if(result == "success")
											{
					...
					layer.confirm("确定要删除该保健员信息？",{icon:3,title:'温馨提示'},function (index) {
						layer.close(index);
						$.ajax({
							url: path + "/admin/deleteHealther",
							type: "post",
							data: {"healtherid": data.healtherid},
							dataType: "text",
							success: function (result) {
		...
		//搜索功能的实现
		$('.healtherTable .layui-btn').on('click', function () {
