		...
		table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
			...
			if (layEvent === 'del') {
				layer.confirm('真的删除行么', function (index) {
					...
					//向服务端发送删除指令
					$.ajax({
						url: src + '/director/delClassTable',
						type: 'post'
						, data: {classid: data.classid,teachername:data.teachername},
				...
				if (method == "dialog") {
					layer.open({
						...
						btn1: function (index, layero) {
							...
							if (formData.classname.length == 0) {
								...
								$.ajax({
									...
									success: function (data) {
										if(data==="success"){
											layer.alert("修改班级成功！",{icon: 6}, function(){
												window.location.href = src + "/director/toUrl/director_ClassManage";
											});
										}else{
											layer.alert("修改班级失败！",{icon: 2});
										}
										layer.close(index);
										}, error: function (err) {
										console.log(err);
						...
						content: src + '/director/toUrl/director_ClassDetailManage' //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
				...
				layer.open({
					...
					btn1: function (index, layero) {
						...
						if (formData.classname.length == 0) {
							...
							$.ajax({
								url: src + '/director/addClassForm',
								type: 'post',
								data: formData,
								success: function (data) {
									if(data==="success"){
										layer.alert("新增班级成功！",{icon: 6}, function(){
											window.location.href = src + "/director/toUrl/director_ClassManage";
										});
									}else{
										layer.alert("新增班级失败！",{icon: 2});
									}
									layer.close(index);
								}, error: function (err) {
									console.log(err);
