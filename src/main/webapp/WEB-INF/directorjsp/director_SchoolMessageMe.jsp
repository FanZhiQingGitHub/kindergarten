...
<div class="demoTable">
	...
	消息内容：
	<div class="layui-inline selects">
		<input class="layui-input" name="infotypename" id="infotypename" autocomplete="off">
	</div>
...
<script>
	layui.use(['table', 'layer', 'form'], function () {
		...
		table.render({
			...
			, url: src + '/director/selectSchoolMessageMe' //数据接口
			, page: true //开启分页
			, id: 'demotable'//当对应的进行条件查询的时候
			, cellMinWidth: 50 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
			, cols: [[ //表头
				{field: 'infotypeid', title: '消息编号', sort: true, align: 'center', width: 120}
				, {field: 'infotypename', title: '消息通知内容', align: 'center', width: 400}
				, {
					field: 'infotypetime', title: '推送时间', align: 'center', width: 180,
					templet: "<div>{{layui.util.toDateString(d.infotypetime,'yyyy-MM-dd HH:mm')}}</div>"
				}
			...
			if (layEvent === 'detail') {
				...
				layer.open({
					...
					content: '<textarea style="padding: 50px; line-height: 22px; ' +
						...
						JSON.stringify(data.infotypename)+'</textarea>'
				});
			}
		});
